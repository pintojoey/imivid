...
public class SurfDetector {
	...
	public static void main(String[] args) {
		...
		Mat testImage = Highgui.imread(test_image, Highgui.CV_LOAD_IMAGE_COLOR);
		...
		Mat outputImage = new Mat(queryImage.rows(), queryImage.cols(), Highgui.CV_LOAD_IMAGE_COLOR);
		Scalar newKeypointColor = new Scalar(255, 0, 0);

		System.out.println("Drawing key points on object image...");
		Features2d.drawKeypoints(queryImage, objectKeyPoints, outputImage, newKeypointColor, 0);

		// Match object image with the scene image
		MatOfKeyPoint sceneKeyPoints = new MatOfKeyPoint();
		MatOfKeyPoint sceneDescriptors = new MatOfKeyPoint();
		System.out.println("Detecting key points in background image...");
		featureDetector.detect(testImage, sceneKeyPoints);
		System.out.println("Computing descriptors in background image...");
		descriptorExtractor.compute(testImage, sceneKeyPoints, sceneDescriptors);

		Mat matchoutput = new Mat(testImage.rows() * 2, testImage.cols() * 2, Highgui.CV_LOAD_IMAGE_COLOR);
